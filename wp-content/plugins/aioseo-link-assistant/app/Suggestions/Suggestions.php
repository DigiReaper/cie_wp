...
class Suggestions {
	...
	public function scheduleMainScan() {
		...
		aioseo()->helpers->scheduleSingleAction( $this->scanActionName, 60 );
	}
	...
	public function registerScan() {
		...
		if ( 401 === $responseCode ) {
			aioseo()->helpers->scheduleSingleAction( $this->registerScanActionName, DAY_IN_SECONDS + wp_rand( 60, 600 ), [], true );
	...
	public function scanPosts() {
		...
		if ( 401 === $responseCode ) {
			aioseo()->helpers->scheduleSingleAction( $this->scanActionName, DAY_IN_SECONDS + wp_rand( 60, 600 ), [], true );
